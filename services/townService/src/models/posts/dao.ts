...
export async function deletePostsBeforeDate(date: Date): Promise<DeletedPostsResponse> {
  const response: DeletedPostsResponse = await PostModel.deleteMany({ createdAt: { $lt: date } });
  return response;
}
