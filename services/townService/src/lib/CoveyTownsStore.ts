...
export default class CoveyTownsStore {
  ...
  private _deleteScheduler: cron.ScheduledTask;

  private constructor() {
    this._deleteScheduler = cron.schedule('0 0 * * * *', () => {
      this._towns.forEach(town => town.deleteBulletinPosts());
    });
  }
  ...
  get deleteScheduler(): cron.ScheduledTask {
    return this._deleteScheduler;
  }
